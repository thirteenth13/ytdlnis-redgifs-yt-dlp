...
# This patch is intentionally limited to TikTokIE (single-post extraction).
# TikTokUserIE profile/feed authentication is handled later by
# patch_tiktok_user_auth.py. Keeping these responsibilities separate makes the
# build resilient to upstream profile extractor changes.
...
print('TikTok: logged-in cookies prefer authenticated web path for single posts; profile auth handled separately')
